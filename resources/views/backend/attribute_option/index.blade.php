...
<div class="card">
    <div class="card-header d-flex justify-content-between align-items-center">
        <h4 class="card-title mb-0">Attribute Options</h4>
...
@include('backend.attribute_option.partials.form')
@endsection
@push('scripts')
<script>
  $(document).ready(function () {
    ...
    const table = $('#DataTable').DataTable({
      processing: true,
      serverSide: true,
      ajax: '{{ route("products.attribute_options", $id) }}',
      columns: [
    ...
    $('#addNew').click(function () {
      $('#modalForm')[0].reset();
      $('#rowId').val('');
      $('.modal-title').text('Add New Attribute Option');
    ...
    $(document).on('click', '.editBtn', function () {
      ...
      const url = '{{ route("attribute-options.edit", ":id") }}'.replace(':id', id);

      $.get(url, function (data) {
        $('#modalForm')[0].reset();
        $('#rowId').val(data.id);
        $('[name="attribute_id"]').val(data.attribute_id).trigger('change');
        $('[name="value"]').val(data.value);
        $('[name="total_stock"]').val(data.total_stock);
        $('[name="price"]').val(data.price);
        $('.modal-title').text('Edit Attribute Option');
        $('#formModal').modal('show');
    ...
    $('#modalForm').submit(function (e) {
      ...
      const url = id
          ? '{{ route("attribute-options.update", ":id") }}'.replace(':id', id)
    ...
    $(document).on('submit', '.delete-form', function (e) {
      ...
      const url = '{{ route("attribute-options.destroy", ":id") }}'.replace(':id', id);
