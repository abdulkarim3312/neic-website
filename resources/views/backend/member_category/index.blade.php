...
$(function () {
    let table = $('#responsive-datatable').DataTable({
        processing: true,
        serverSide: true,
        ajax: '{{ route("member-categories.index") }}',
        columns: [
    ...
    $(document).on('click', '.editBtn', function () {
        ...
        const url = '{{ route("member-categories.edit", ":id") }}'.replace(':id', id);

        $.get(url, function (data) {
            $('#modalForm')[0].reset();
            $('#rowId').val(data.id);
            $('[name="name_bn"]').val(data.name_bn);
    ...
    $('#modalForm').submit(function (e) {
        ...
        let id = $('#rowId').val();
        let url = id
            ? '{{ route("member-categories.update", ":id") }}'.replace(':id', id)
            : '{{ route("member-categories.store") }}';
    ...
    $(document).on('submit', '.delete-form', function (e) {
        ...
        const url = '{{ route("member-categories.destroy", ":id") }}'.replace(':id', id);
    ...
    $(document).on('change', '.status-toggle', function () {
        ...
        const url = '{{ route("member_status", ":id") }}'.replace(':id', id);
